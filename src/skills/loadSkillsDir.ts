...
export function getSkillsPath(
  ...
  switch (source) {
    case 'policySettings':
      return join(getManagedFilePath(), '.legna', dir)
    case 'userSettings':
...
/**
 * Parse paths frontmatter from a skill, using the same format as LEGNA.md rules.
...
export const getSkillDirCommands = memoize(
  async (cwd: string): Promise<Command[]> => {
    ...
    const managedSkillsDir = join(getManagedFilePath(), '.legna', 'skills')
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      const skillDir = join(currentDir, '.legna', 'skills')
