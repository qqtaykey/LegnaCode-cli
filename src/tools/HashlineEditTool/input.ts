/**
 * Hashline input splitting — parses §PATH headers to split multi-file patches.
 */

import * as path from 'node:path'
import { BEGIN_PATCH_MARKER, END_PATCH_MARKER, ABORT_MARKER } from './constants'
...
function containsRecognizableHashlineOperations(input: string): boolean {
  return input.split(/\r?\n/).some(line => HASHLINE_OP_LINE_RE.test(line))
}
