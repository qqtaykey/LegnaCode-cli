/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with LegnaCode CLI-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and LegnaCode CLI's
...
/**
 * Resolve LegnaCode-specific path patterns for sandbox-runtime.
 *
 * LegnaCode uses special path prefixes in permission rules:
...
/**
 * Convert LegnaCode settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // This blocks settings in the original working directory (where LegnaCode started)
  ...
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.legna', 'settings.json'))
    denyWrite.push(resolve(cwd, '.legna', 'settings.local.json'))
    denyWrite.push(resolve(cwd, '.claude', 'settings.json'))
    denyWrite.push(resolve(cwd, '.claude', 'settings.local.json'))
  }

  // Block writes to .legna/skills and .claude/skills in both original and current working directories.
  ...
  denyWrite.push(resolve(originalCwd, '.legna', 'skills'))
  denyWrite.push(resolve(originalCwd, '.claude', 'skills'))
  if (cwd !== originalCwd) {
    denyWrite.push(resolve(cwd, '.legna', 'skills'))
 ...
 * This is a LegnaCode CLI-specific function that updates local settings.
 */
...
/**
 * LegnaCode CLI sandbox manager - wraps sandbox-runtime with Claude-specific features
 */
