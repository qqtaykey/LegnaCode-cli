...
export type GlobalConfig = {
  ...
  firstStartTime?: string // ISO timestamp when LegnaCode was first started on this machine
  ...
  // LegnaCode usage tracking
  claudeCodeFirstTokenDate?: string // ISO timestamp of the user's first LegnaCode OAuth token
  ...
  // LegnaCode in Chrome settings
  hasCompletedClaudeInChromeOnboarding?: boolean // Whether LegnaCode in Chrome onboarding has been shown
  claudeInChromeDefaultEnabled?: boolean // Whether LegnaCode in Chrome is enabled by default (undefined means platform default)
  ...
  // LegnaCode hint protocol state (<claude-code-hint /> tags from CLIs/SDKs).
...
function saveConfigWithLock<A extends object>(
  ...
  try {
    ...
    if (lockTime > 100) {
      logForDebugging(
        'Lock acquisition took longer than expected - another LegnaCode instance may be running',
      )
...
export function getMemoryPath(memoryType: MemoryType): string {
  ...
  switch (memoryType) {
    case 'User':
      return join(getClaudeConfigHomeDir(), 'LEGNA.md')
    case 'Local':
      return join(cwd, 'LEGNA.local.md')
    case 'Project':
      return join(cwd, 'LEGNA.md')
    case 'Managed':
      return join(getManagedFilePath(), 'LEGNA.md')
    case 'AutoMem':
