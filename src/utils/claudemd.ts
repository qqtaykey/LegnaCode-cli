...
/**
 * Checks whether a LEGNA.md file path is excluded by the claudeMdExcludes setting.
...
function isClaudeMdExcluded(filePath: string, type: MemoryType): boolean {
  ...
  // the user writes "/tmp/project/LEGNA.md" in their exclude, but the system
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    for (const dir of dirs.reverse()) {
      ...
      // Try reading LEGNA.md (Project) - only if projectSettings is enabled
      ...
      // Try reading LEGNA.local.md (Local) - only if localSettings is enabled
      if (isSettingSourceEnabled('localSettings')) {
        const localPath = join(dir, 'LEGNA.local.md')
        result.push(
          ...(await processMemoryFile(
            localPath,
            'Local',
    ...
    // Process LEGNA.md from additional directories (--add-dir) if env var is enabled
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        // Try reading LEGNA.md from the additional directory
        const projectPath = join(dir, 'LEGNA.md')
        result.push(
          ...(await processMemoryFile(
            projectPath,
    ...
    // memory system, not "instructions" in the LEGNA.md/rules sense.
 ...
 * Loads LEGNA.md, unconditional rules, and conditional rules for that directory.
 *
...
export async function getMemoryFilesForNestedDirectory(
  ...
  // Process local memory file (LEGNA.local.md)
  if (isSettingSourceEnabled('localSettings')) {
    const localPath = join(dir, 'LEGNA.local.md')
    result.push(
...
export function isMemoryFilePath(filePath: string): boolean {
  ...
  // LEGNA.md or LEGNA.local.md anywhere
  if (name === 'LEGNA.md' || name === 'LEGNA.local.md') {
    return true
  }
