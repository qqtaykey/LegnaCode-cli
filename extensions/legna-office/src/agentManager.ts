    ...
    try {
      if (fs.existsSync(projectsRoot)) {
        ...
        if (match && match !== dirName) {
          ...
          console.log(
            `[LegnaCode Office] Project dir not found, using case-insensitive match: ${dirName} → ${match}`,
          );
          return matchedDir;
        }
        if (!match) {
          console.warn(
            `[LegnaCode Office] Project dir does not exist: ${projectDir}. ` +
...
export async function launchNewTerminal(
  ...
  persistAgents();
  console.log(`[LegnaCode Office] Terminal: Agent ${id} - created for terminal ${terminal.name}`);
  ...
  let pollCount = 0;
  console.log(`[LegnaCode Office] Terminal: Agent ${id} - waiting for JSONL at ${agent.jsonlFile}`);
  const pollTimer = setInterval(() => {
    pollCount++;
    try {
      if (fs.existsSync(agent.jsonlFile)) {
        console.log(
          `[LegnaCode Office] Terminal: Agent ${id} - found JSONL file ${path.basename(agent.jsonlFile)} (after ${pollCount}s)`,
        );
        ...
        try {
          ...
          if (candidates.length > 0) {
            console.log(
              `[LegnaCode Office] Terminal: Agent ${id} - /resume detected, reassigning to ${path.basename(candidates[0].file)}`,
            );
            clearInterval(pollTimer);
            jsonlPollTimers.delete(id);
            reassignAgentToFile(
...
export function restoreAgents(
  ...
  for (const p of persisted) {
    ...
    if (isExternal) {
      console.log(
        `[LegnaCode Office] Terminal: Agent ${p.id} - restored external → ${path.basename(p.jsonlFile)}`,
      );
    } else {
      console.log(
        `[LegnaCode Office] Terminal: Agent ${p.id} - restored → terminal "${p.terminalName}"`,
      );
    }
    ...
    try {
      if (fs.existsSync(p.jsonlFile)) {
        ...
        const pollTimer = setInterval(() => {
          try {
            if (fs.existsSync(agent.jsonlFile)) {
              console.log(`[LegnaCode Office] Terminal: Agent ${p.id} - found JSONL file`);
              clearInterval(pollTimer);
    ...
    setTimeout(() => {
      for (const id of restoredTerminalIds) {
        ...
        if (agent && !agent.isExternal && agent.linesProcessed === 0) {
          console.log(
            `[LegnaCode Office] Terminal: Agent ${id} - removing restored agent, no data received`,
          );
...
export function sendExistingAgents(
  ...
  console.log(
    `[LegnaCode Office] sendExistingAgents: agents=${JSON.stringify(agentIds)}, meta=${JSON.stringify(agentMeta)}`,
  );
