...
export class LegnaOfficeViewProvider implements vscode.WebviewViewProvider {
  ...
  // LegnaCode Office Server (hook event reception)
  private legnaOfficeServer: LegnaOfficeServer | null = null;
  // ServerConfig is not stored as a field; use this.legnaOfficeServer?.getConfig() if needed.
  ...
  private initHooks(): void {
    ...
    this.legnaOfficeServer = new LegnaOfficeServer();
    this.legnaOfficeServer.onHookEvent((providerId, event) => {
      this.hookEventHandler?.handleEvent(providerId, event as HookEvent);
    });

    this.legnaOfficeServer
      .start()
      .then((config) => {
      ...
      .catch((e) => {
        console.error(`[LegnaCode Office] Failed to start server: ${e}`);
      });
  }
  ...
  private removeTeammate(teammateAgentId: number, source: string): void {
    ...
    if (!agent) return;
    console.log(`[LegnaCode Office] Removing teammate ${teammateAgentId} (source: ${source})`);
    dismissedJsonlFiles.set(agent.jsonlFile, Date.now());
    this.unregisterAgentHook(agent);
    removeAgent(
      teammateAgentId,
  ...
  private removeTeammates(leadId: number): void {
    ...
    for (const id of teammates) {
      ...
      if (agent) {
        console.log(`[LegnaCode Office] Removing teammate ${id} (lead ${leadId} closed)`);
        dismissedJsonlFiles.set(agent.jsonlFile, Date.now());
        this.unregisterAgentHook(agent);
        removeAgent(
          id,
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'openClaude') {
        ...
        console.log(`[LegnaCode Office] State: saveAgentSeats:`, JSON.stringify(message.seats));
        ...
        if (enabled) {
          installHooks();
          copyHookScript(this.context.extensionPath);
          console.log('[LegnaCode Office] Hooks enabled by user');
        } else {
          uninstallHooks();
          console.log('[LegnaCode Office] Hooks disabled by user');
        }
        ...
        console.log(`[LegnaCode Office] Debug: Platform: ${process.platform}, arch: ${process.arch}`);
          ...
          if (wsFolders && wsFolders.length > 1) {
            for (const folder of wsFolders) {
              ...
              if (folderProjectDir && folderProjectDir !== projectDir) {
                console.log(
                  `[LegnaCode Office] Registering additional project dir: ${folderProjectDir}`,
                );
                ensureProjectScan(
                  folderProjectDir,
                  this.knownJsonlFiles,
        ...
        if (!layout) {
          vscode.window.showWarningMessage('LegnaCode Office: No saved layout to export.');
          return;
        }
        const uri = await vscode.window.showSaveDialog({
          filters: { 'JSON Files': ['json'] },
          defaultUri: vscode.Uri.file(path.join(os.homedir(), 'legna-office-layout.json')),
        });
        if (uri) {
          fs.writeFileSync(uri.fsPath, JSON.stringify(layout, null, 2), 'utf-8');
          vscode.window.showInformationMessage('LegnaCode Office: Layout exported successfully.');
        }
        ...
        try {
          ...
          if (imported.version !== 1 || !Array.isArray(imported.tiles)) {
            vscode.window.showErrorMessage('LegnaCode Office: Invalid layout file.');
            return;
          }
          this.layoutWatcher?.markOwnWrite();
          writeLayoutToFile(imported);
          this.webview?.postMessage({ type: 'layoutLoaded', layout: imported });
          vscode.window.showInformationMessage('LegnaCode Office: Layout imported successfully.');
        } catch {
          vscode.window.showErrorMessage('LegnaCode Office: Failed to read or parse layout file.');
        }
      }
    });
  ...
  exportDefaultLayout(): void {
    const layout = readLayoutFromFile();
    if (!layout) {
      vscode.window.showWarningMessage('LegnaCode Office: No saved layout found.');
      return;
    }
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!workspaceRoot) {
      vscode.window.showErrorMessage('LegnaCode Office: No workspace folder found.');
      return;
    }
    ...
    vscode.window.showInformationMessage(
      `LegnaCode Office: Default layout exported as revision ${nextRevision} to ${targetPath}`,
    );
  }
  ...
  private startLayoutWatcher(): void {
    ...
    this.layoutWatcher = watchLayoutFile((layout) => {
      console.log('[LegnaCode Office] External layout change — pushing to webview');
      this.webview?.postMessage({ type: 'layoutLoaded', layout });
    });
  }

  dispose() {
    this.legnaOfficeServer?.stop();
    this.legnaOfficeServer = null;
