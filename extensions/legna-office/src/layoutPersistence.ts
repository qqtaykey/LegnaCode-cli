...
export function readLayoutFromFile(): Record<string, unknown> | null {
  ...
  try {
    ...
    console.error('[LegnaCode Office] Failed to read layout file:', err);
    return null;
  }
}
...
export function writeLayoutToFile(layout: Record<string, unknown>): void {
  ...
  try {
    ...
    console.error('[LegnaCode Office] Failed to write layout file:', err);
  }
}
...
export function migrateAndLoadLayout(
  ...
  if (fromFile) {
    ...
    if (defaultRevision > fileRevision) {
      console.log(
        `[LegnaCode Office] Layout revision outdated (${fileRevision} < ${defaultRevision}), resetting to bundled default`,
      );
      writeLayoutToFile(defaultLayout!);
      return { layout: defaultLayout!, wasReset: true };
    }
    console.log('[LegnaCode Office] Layout loaded from file');
  ...
  if (fromState) {
    console.log('[LegnaCode Office] Migrating layout from workspace state to file');
  ...
  if (defaultLayout) {
    console.log('[LegnaCode Office] Writing bundled default layout to file');
    writeLayoutToFile(defaultLayout);
...
/**
 * Watch ~/.legna-office/layout.json for external changes (other VS Code windows).
...
export function watchLayoutFile(
  ...
  function checkForChange(): void {
    ...
    try {
      ...
      console.log('[LegnaCode Office] External layout change detected');
      onExternalChange(layout);
    } catch (err) {
      console.error('[LegnaCode Office] Error checking layout file:', err);
    }
  }
  ...
  function startFsWatch(): void {
    ...
    try {
      ...
      fsWatcher.on('error', (err) => {
        ...
        console.log(`[LegnaCode Office] Layout: fs.watch error: ${err.message}`);
        fsWatcher?.close();
        fsWatcher = null;
      });
    } catch {
