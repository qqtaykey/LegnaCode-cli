    ...
    if (
      ...
      try {
        ...
        for (const file of dirFiles) {
          ...
          console.log(
            `[LegnaCode Office] Watcher: Agent ${agentId} - /clear detected, reassigning to ${path.basename(file)}`,
          );
          reassignAgentToFile(
            agentId,
            file,
...
export function readNewLines(
  ...
  try {
    ...
    console.log(`[LegnaCode Office] Watcher: Agent ${agentId} - read error: ${e}`);
  }
}
...
function scanForNewJsonlFiles(
  ...
  for (const [id, agent] of agents) {
    ...
    if (agent.terminalRef && agent.terminalRef.exitStatus !== undefined) {
      console.log(`[LegnaCode Office] Watcher: Agent ${id} - terminal closed, cleaning up orphan`);
      // Stop file watching
...
function adoptTerminalForFile(
  ...
  console.log(
    `[LegnaCode Office] Watcher: Agent ${id} - adopted terminal "${terminal.name}" for ${path.basename(jsonlFile)}`,
  );
...
export function scanForTeammateFiles(
  ...
  for (const file of files) {
    ...
    if (existingTeammate) {
      if (debug)
        console.log(
          `[LegnaCode Office] Teammate "${teammateName}" already exists (Agent ${existingTeammate.id}), reassigning to ${path.basename(file)}`,
        );
    ...
    console.log(
      `[LegnaCode Office] Teammate detected: "${teammateName}" (Agent ${id}) for parent Agent ${parentAgentId} (${path.basename(file)})`,
    );
...
export function adoptExternalSessionFromHook(
  ...
  if (transcriptPath) {
    ...
    if (adoptedAgent && debug) {
      console.log(
        `[LegnaCode Office] Hook: Agent ${adoptedAgent.id} - detected external session ${path.basename(transcriptPath)}${adoptedAgent.folderName ? ` (${adoptedAgent.folderName})` : ''}`,
      );
    }
    if (adoptedAgent) {
    ...
    if (debug) {
      console.log(
        `[LegnaCode Office] Hook: Agent ${id} - detected hooks-only external session${folderName ? ` (${folderName})` : ''}`,
      );
    }
...
function scanExternalDir(
  ...
  for (const file of files) {
    ...
    console.log(`[LegnaCode Office] Watcher: detected external session ${path.basename(file)}`);
    adoptExternalSession(
      file,
      projectDir,
      nextAgentIdRef,
...
function scanGlobalProjectDirs(
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      knownJsonlFiles.add(file);
      console.log(
        `[LegnaCode Office] Watcher: detected global session ${path.basename(file)} (${folderName})`,
      );
      adoptExternalSession(
        file,
        dirPath,
...
export function startStaleExternalAgentCheck(
  ...
  return setInterval(() => {
    ...
    for (const id of toRemove) {
      ...
      console.log(`[LegnaCode Office] Watcher: Agent ${id} - removing stale external agent`);
      removeAgent(
        id,
        agents,
        fileWatchers,
