...
/** Marker string used to identify LegnaCode Office hook entries in Claude's settings. */
...
/** Returns the destination path for the hook script (~/.legna-office/hooks/claude-hook.js). */
...
function readClaudeSettings(): ClaudeSettings {
  ...
  try {
    ...
    console.error(`[LegnaCode Office] Failed to read Claude settings: ${e}`);
  }
  return {};
}
...
function writeClaudeSettings(settings: ClaudeSettings): void {
  ...
  try {
    ...
    const tmpPath = settingsPath + '.legna-office-tmp';
    fs.writeFileSync(tmpPath, JSON.stringify(settings, null, 2), 'utf-8');
    fs.renameSync(tmpPath, settingsPath);
  } catch (e) {
    console.error(`[LegnaCode Office] Failed to write Claude settings: ${e}`);
  }
}

/** Legacy script name (before rename to claude-hook.js). */
const LEGACY_HOOK_MARKER = 'legna-office-hook.js';

/** Check if a hook entry belongs to LegnaCode Office (current or legacy script name). */
...
/** Check if LegnaCode Office hooks are already installed in ~/.claude/settings.json. */
...
/**
 * Install LegnaCode Office hook entries into ~/.claude/settings.json for
 * Notification, Stop, and PermissionRequest events. Idempotent: removes
 * any existing LegnaCode Office entries before adding fresh ones.
 */
export function installHooks(): void {
  ...
  for (const event of events) {
    ...
    const entries = settings.hooks[event];
    // Remove any existing LegnaCode Office entries (in case script path changed)
  ...
  if (changed) {
    ...
    console.log('[LegnaCode Office] Hooks installed in ~/.claude/settings.json');
  }
}

/** Remove all LegnaCode Office hook entries from ~/.claude/settings.json. Cleans up empty objects. */
export function uninstallHooks(): void {
  ...
  if (changed) {
    writeClaudeSettings(settings);
    console.log('[LegnaCode Office] Hooks removed from ~/.claude/settings.json');
  }
}

/** Copy the shipped hook script from the extension to ~/.legna-office/hooks/ */
export function copyHookScript(extensionPath: string): void {
  ...
  try {
    ...
    if (!fs.existsSync(src)) {
      console.warn(`[LegnaCode Office] Hook script not found at ${src}`);
      return;
    }
    fs.copyFileSync(src, dst);
    fs.chmodSync(dst, 0o700);
    console.log(`[LegnaCode Office] Hook script installed at ${dst}`);
  } catch (e) {
    console.error(`[LegnaCode Office] Failed to copy hook script: ${e}`);
  }
}
