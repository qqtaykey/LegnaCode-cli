...
const debug = process.env.LEGNA_OFFICE_DEBUG !== '0';
...
export class HookEventHandler {
  ...
  handleEvent(_providerId: string, event: HookEvent): void {
    ...
    if (normEvent.kind === 'sessionStart') {
      ...
      if (debug && tracked)
        console.log(`[LegnaCode Office] Hook: SessionStart(source=${source}, session=${sid}...)`);
      ...
      if (existingAgentId !== undefined) {
        ...
        if (debug)
          console.log(
            `[LegnaCode Office] Hook: Agent ${existingAgentId} - SessionStart(source=${source}) known`,
          );
        return;
      }
      ...
      for (const [id, agent] of this.agents) {
        if (agent.sessionId === event.session_id) {
          ...
          if (debug)
            console.log(
              `[LegnaCode Office] Hook: Agent ${id} - SessionStart(source=${source}) auto-discovered`,
            );
          return;
        }
      }
        ...
        if (projectDir) {
          for (const [id, agent] of this.agents) {
            ...
            if (isMatch) {
              ...
              console.log(
                `[LegnaCode Office] Hook: Agent ${id} - /${normEvent.source} detected, reassigning to ${event.session_id}`,
              );
      ...
      if (transcriptPath || cwd) {
        ...
        if (debug && tracked)
          console.log(
            `[LegnaCode Office] Hook: SessionStart(source=${source}) -> pending external session ${sid}..., awaiting confirmation`,
          );
        ...
        if (debug && tracked)
          console.log(
            `[LegnaCode Office] Hook: SessionStart -> unknown session ${sid}..., no transcript_path`,
          );
      }
      return;
    }
    ...
    if (normEvent.kind === 'sessionEnd' && this.pendingExternalSessions.has(event.session_id)) {
      ...
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: SessionEnd discarded pending external session ${event.session_id.slice(0, 8)}...`,
        );
      return;
    }
    ...
    if (this.pendingExternalSessions.has(event.session_id)) {
      ...
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: ${eventName} confirmed external session ${event.session_id.slice(0, 8)}..., creating agent`,
        );
    ...
    if (agentId === undefined) {
      ...
      if (isPending || hasBuffered || hasUnregisteredAgents) {
        if (debug)
          console.log(
            `[LegnaCode Office] Hook: ${eventName} - unknown session ${event.session_id.slice(0, 8)}..., buffering`,
          );
    ...
    if (debug)
      console.log(
        `[LegnaCode Office] Hook: Agent ${agentId} - ${eventName} (session=${event.session_id.slice(0, 8)}...)`,
      );
  ...
  private handleSessionEnd(
    ...
    if (debug)
      console.log(
        `[LegnaCode Office] Hook: Agent ${agentId} - SessionEnd(reason=${reason ?? 'unknown'})`,
      );
    ...
    if (expectsFollowUp) {
      ...
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: Agent ${agentId} - SessionEnd(reason=${reason}), awaiting possible SessionStart`,
        );
  ...
  private handleSubagentStart(
    ...
    if (this.provider.team && agent.currentHookIsTeammateSpawn === true && agent.teamName) {
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: Agent ${agentId} - SubagentStart: teammate "${agentType}" detected, triggering discovery`,
        );
  ...
  private handleSubagentStop(
    ...
    if (inlineTeammates.length > 0) {
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: Agent ${agentId} - SubagentStop: marking inline teammates as waiting`,
        );
  ...
  private handleTeammateIdle(
    ...
    if (agentType) {
      ...
      if (match) {
        const [id, a] = match;
        if (debug)
          console.log(`[LegnaCode Office] Hook: TeammateIdle "${agentType}" -> teammate Agent ${id}`);
        this.markAgentWaiting(a, id, webview);
        return;
      }
    }
    ...
    if (debug)
      console.log(
        `[LegnaCode Office] Hook: TeammateIdle (no agent_type match) -> marking ${inlineTeammates.length} teammate(s) waiting`,
      );
    for (const [id, a] of inlineTeammates) {
      this.markAgentWaiting(a, id, webview);
    }
  ...
  private handleTaskCompleted(event: HookEvent, agentId: number): void {
    ...
    if (debug)
      console.log(
        `[LegnaCode Office] Hook: Agent ${agentId} - TaskCompleted: ${subject}${agentType ? ` (agent_type=${agentType})` : ''}`,
      );
  ...
  private flushBufferedEvents(sessionId: string): void {
    ...
    if (debug && toFlush.length > 0) {
      if (debug)
        console.log(
          `[LegnaCode Office] Hook: flushing ${toFlush.length} buffered event(s) for session ${sessionId.slice(0, 8)}...`,
        );
    }
